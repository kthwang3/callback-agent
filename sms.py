...
async def send_sms(call_data: dict) -> None:
    ...
    message = await asyncio.to_thread(client.messages.create,
            body=f"Missed Patient Call from: {call_data["caller_name"]} Number: {call_data["callback_number"]} Reason: {call_data["reason"]} New or Returning: {new_or_returning} Day Preference: {day_preference}",
